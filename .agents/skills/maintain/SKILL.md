---
name: maintain
description: Detect and fix stale brain/*.md knowledge docs. Use when brain/architecture.md or build.md references missing files, wrong commands, or outdated patterns. Keeps the knowledge base accurate.
---
...
- When a command in brain/build.md produces "command not found"
- When brain/architecture.md references a directory that no longer exists
- Periodically as part of session end

## Protocol

### 1. Scan brain/*.md files for staleness signals
Read each brain/*.md file and flag:
...
If you discover new facts about the codebase during maintenance, add them to the appropriate brain/*.md file.
...
- Document the maintenance run in brain/tasks/lessons.md
