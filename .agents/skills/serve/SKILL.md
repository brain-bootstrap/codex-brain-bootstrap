---
name: serve
description: Start service(s) locally for development. Accepts a service name, all, frontend, or backend. Reads brain/build.md for the actual start commands.
---
...
**First:** Read `brain/build.md` for local development setup and service definitions.
...
| (empty) | Show available services from `brain/build.md` |

### Rules:
- Run services in background (async) so the session stays responsive
- After starting, show the expected URLs (from `brain/build.md`)
...
### Example (if brain/build.md has commands):
```bash
# Backend
pnpm --filter api dev &
