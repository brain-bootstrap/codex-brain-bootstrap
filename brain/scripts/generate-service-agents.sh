#!/bin/bash
# generate-service-agents.sh — Generate AGENTS.md stubs for each service in a monorepo
# Usage: bash brain/scripts/generate-service-agents.sh [root_dir]
# Scans common monorepo service directories and creates minimal AGENTS.md stubs.
...
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  echo "❌ generate-service-agents.sh must be EXECUTED, not sourced." >&2
...
# Create an AGENTS.md stub for a service directory
create_stub() {
  local service_dir="${1%/}"
  local SERVICE_NAME
  ...
  { [ -d "${service_dir}/test" ] || [ -d "${service_dir}/tests" ] || [ -d "${service_dir}/__tests__" ]; } && \
    KEY_FILES="$KEY_FILES\n- \`test/\` — tests"
  ...
  CONTENT="$CONTENT\n\n## Notes\n<!-- Add service-specific conventions, gotchas, and domain rules here -->"
...
for parent in "${SERVICE_PARENTS[@]}"; do
  ...
  for service_dir in "$PARENT_DIR"/*/; do
    ...
    if has_manifest "$service_dir"; then
      ...
      # Direct child has no manifest — likely a container dir
