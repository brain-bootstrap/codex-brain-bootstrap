...
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  ...
  echo "   Wrong:  source brain/scripts/discover.sh" >&2
  echo "   Right:  bash brain/scripts/discover.sh [project-dir]" >&2
...
emit "HAS_SCRIPTS"     "$([ -d "brain/scripts" ] && [ -f "brain/scripts/discover.sh" ] && echo true || echo false)"

# Mode decision
# UPGRADE only if config exists AND has been bootstrapped.
#
# PROBLEM: {{PROJECT_NAME}} in AGENTS.md is an UNRELIABLE FRESH signal.
# When a user copies a new template over an existing bootstrapped config,
# AGENTS.md gets the template version (with {{PROJECT_NAME}}) even though
...
#
#   1. brain/tasks/lessons.md with real content
#      The template ships lessons.md with only a placeholder header.
#      Real content means bootstrap ran previously.
#
#   2. brain/tasks/CODEX_ERRORS.md WITHOUT {{PROJECT_NAME}}
#      The template always ships CODEX_ERRORS.md WITH {{PROJECT_NAME}}.
#      Once bootstrap runs, the placeholder is replaced → no placeholder = was bootstrapped.
#
# Priority:
#   1. CODEX_ERRORS.md bootstrapped (no {{...}})   → UPGRADE  (bootstrap ran previously)
#   2. Neither above, but {{PROJECT_NAME}} in AGENTS.md → FRESH (genuine new install)
#   3. Neither above, no placeholder               → UPGRADE  (hand-crafted or prior bootstrap)

_errors_bootstrapped=false
if [ -f "brain/tasks/CODEX_ERRORS.md" ] && ! grep -q '{{PROJECT_NAME}}' "brain/tasks/CODEX_ERRORS.md" 2>/dev/null; then
  _errors_bootstrapped=true
fi

if [ -f "AGENTS.md" ] || [ -d ".codex" ] || [ -d "brain" ]; then
  if [ "$_errors_bootstrapped" = "true" ]; then
    emit "MODE" "UPGRADE"
  elif [ -f "AGENTS.md" ] && grep -q '{{PROJECT_NAME}}' AGENTS.md 2>/dev/null; then
    emit "MODE" "FRESH"
  else
    emit "MODE" "UPGRADE"
  fi
...
# .cocoindex_code/ data directory present
HAS_COCOINDEX_DATA="false"
[ -d ".cocoindex_code" ] && HAS_COCOINDEX_DATA="true"

emit "HAS_GRAPHIFY" "$HAS_GRAPHIFY"
emit "HAS_RTK" "$HAS_RTK"
emit "HAS_CBM" "$HAS_CBM"
emit "HAS_COCOINDEX" "$HAS_COCOINDEX"
emit "HAS_COCOINDEX_DATA" "$HAS_COCOINDEX_DATA"
