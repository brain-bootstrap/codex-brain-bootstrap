#!/usr/bin/env bash
# merge-agents-md.sh — Deterministic AGENTS.md section merger
...
#
# Usage: bash brain/scripts/merge-agents-md.sh --template <file> --target <file> [--dry-run]
# Exit:  0 = success, 1 = error, 2 = nothing to add

# ─── Source guard ─────────────────────────────────────────────────
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  echo "❌ merge-agents-md.sh must be EXECUTED, not sourced." >&2
...
if [ "$ADDED" -eq 0 ]; then
  echo ""
  echo "✅ AGENTS.md: all template sections already covered ($SKIPPED matched)"
  exit 0
fi

# Budget check (Codex 32KB limit — warn at 600 lines)
...
if [ "$TOTAL" -gt 600 ]; then
  echo ""
  echo "  ⚠️  BUDGET WARNING: result would be $TOTAL lines (>600). Consider offloading to brain/*.md"
fi
